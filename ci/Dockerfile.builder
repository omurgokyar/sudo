...
# sudo_hash
RUN git clone https://github.com/cryptosudo/sudo_hash
RUN cd sudo_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} sudo
RUN useradd -u ${USER_ID} -g sudo -s /bin/bash -m -d /sudo sudo
...
RUN mkdir /sudo-src && \
  ...
  chown $USER_ID:$GROUP_ID /sudo-src && \
