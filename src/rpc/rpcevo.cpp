...
// Allows to specify Sudo address or priv key. In case of Sudo address, the priv key is taken from the wallet
...
void protx_register_fund_help()
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 50000 SUDO\n"
            "to the address specified by collateralAddress and will then function as the collateral of your\n"
            "masternode.\n"
            "A few of the limitations you see in the arguments are temporary and might be lifted after DIP3\n"
            "is fully deployed.\n"
            "\nArguments:\n"
            "1. \"collateralAddress\"   (string, required) The sudo address to send the collateral to.\n"
            ...
            "7. \"payoutAddress\"       (string, required) The sudo address to use for masternode reward payments.\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Sudo address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() >= 6) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Sudo address: ") + request.params[5].get_str());
...
void protx_update_registrar_help()
{
    throw std::runtime_error(
            ...
            "4. \"payoutAddress\"       (string, required) The sudo address to use for masternode reward payments\n"
            "                         If set to an empty string, the last on-chain payout address of the masternode will be used.\n"
            "5. \"feeSourceAddress\"    (string, optional) If specified wallet will only use coins from this address to fund ProTx.\n"
            "                         If not specified, payoutAddress is the one that is going to be used.\n"
            "                         The private key belonging to this address must be known in your wallet.\n"
            "\nExamples:\n"
            + HelpExampleCli("protx", "update_registrar \"0123456701234567012345670123456701234567012345670123456701234567\" \"982eb34b7c7f614f29e5c665bc3605f1beeef85e3395ca12d3be49d2868ecfea5566f11cedfad30c51b2403f2ad95b67\" \"SwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG\"")
    );
}
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Sudo address: ") + request.params[4].get_str());
        FundSpecialTx(tx, ptx, feeSourceAddress.Get());
