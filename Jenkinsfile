...
// which are part of the Sudo repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="sudo-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /sudo-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "sudopay-sudo/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from sudopay-sudo/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'sudopay-sudo/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from sudopay-sudo/develop"
            }
          }

          if (hasCache) {
            sh "cd /sudo-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /sudo-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /sudo-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /sudo-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /sudo-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /sudo-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /sudo-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
